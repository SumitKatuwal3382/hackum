...
export default function StudentOnboarding({ onCreated, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-4 bg-[rgba(255,255,255,0.01)] border border-[rgba(255,255,255,0.02)] rounded-xl">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-100">Add Yourself</h2>
        <div className="flex gap-2">
          <button type="button" onClick={onCancel} className="px-3 py-1 text-sm rounded-lg border border-[rgba(255,255,255,0.04)] text-gray-200">Cancel</button>
          <button type="submit" className="px-4 py-1.5 text-sm rounded-lg bg-[linear-gradient(180deg,#3b82f6,#0ea5ff)] text-white">Save</button>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div>
          ...
          <input id="fullName" value={name} onChange={e=>setName(e.target.value)} className="w-full px-3 py-2 border border-[rgba(255,255,255,0.03)] rounded-lg text-sm bg-transparent text-gray-200" placeholder="Jane Doe" />
        </div>
        <div>
          <label htmlFor="major" className="block text-xs font-medium text-gray-400 mb-1">Major</label>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
            <h3 className="font-medium text-gray-100 text-sm">Courses</h3>
          <Badge>{selectedCourses.length} selected</Badge>
        </div>
        <div className="flex flex-wrap gap-2">
          {courses.map(c => (
            <button key={c.id} type="button" onClick={()=>toggleCourse(c.id)} className={`px-3 py-1 rounded-full text-xs border ${selectedCourses.includes(c.id) ? 'bg-[linear-gradient(180deg,#3b82f6,#0ea5ff)] text-white border-[rgba(59,130,246,0.6)]' : 'bg-[rgba(255,255,255,0.01)] text-gray-200 border-[rgba(255,255,255,0.02)]'}`}>{c.id}</button>
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-medium text-gray-100 text-sm">Weak Concepts</h3>
          <Badge>{weakConcepts.length} selected</Badge>
        </div>
        <div className="flex flex-wrap gap-2 mb-3">
          {concepts.map(k => (
            <button key={k.id} type="button" onClick={()=>toggleWeakConcept(k.id)} className={`px-3 py-1 rounded-full text-xs border ${weakConcepts.some(w=>w.concept_id===k.id) ? 'bg-[rgba(34,197,94,0.9)] text-white border-[rgba(34,197,94,0.35)]' : 'bg-[rgba(255,255,255,0.01)] text-gray-200 border-[rgba(255,255,255,0.02)]'}`}>{k.name}</button>
          ))}
        </div>
      ...
      <div>
          <div className="flex items-center justify-between mb-2">
          <h3 className="font-medium text-gray-100 text-sm">Availability</h3>
          <button type="button" onClick={addAvailRow} className="px-2 py-1 text-xs rounded-md border border-[rgba(255,255,255,0.03)] text-gray-200">Add Slot</button>
        </div>
        ...
        <div className="space-y-2">
          {availabilityRows.map((r,i)=>(
            <div key={i} className="flex items-center gap-2 text-xs">
              <select value={r.day} onChange={e=>updateAvail(i,'day',e.target.value)} className="border border-[rgba(255,255,255,0.03)] rounded px-2 py-1 bg-transparent text-gray-200">
                {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d=> <option key={d}>{d}</option>)}
              </select>
              <input type="number" min={0} max={23} value={r.hour_start} onChange={e=>updateAvail(i,'hour_start',e.target.value)} className="w-16 border border-[rgba(255,255,255,0.03)] rounded px-2 py-1 bg-transparent text-gray-200" />
              <span>to</span>
              <input type="number" min={0} max={23} value={r.hour_end} onChange={e=>updateAvail(i,'hour_end',e.target.value)} className="w-16 border border-[rgba(255,255,255,0.03)] rounded px-2 py-1 bg-transparent text-gray-200" />
              <button type="button" onClick={()=>removeAvail(i)} className="text-red-400 hover:underline">Remove</button>
            </div>
          ))}
        </div>
      </div>
